...
class lqw_learner_lp_statement_of_result(report_sxw.rml_parse):

	def __init__(self, cr, uid, name, context=None):
		super(lqw_learner_lp_statement_of_result, self).__init__(cr, uid, name, context=context)
	...
	def get_lp(self, achieved_id):
		...
		dbg("get_lp")
		self.cr.execute(
			"select learning_programme_id from learning_programme_learner_rel where id=%s" % (achieved_id.id))
	...
	def get_unit_standard(self, achieved_id):
		...
		# self.cr.execute(
		# 	"select id from learner_registration_qualification_line where learner_reg_id=%s" % (
		# 		achieved_id.id))
		self.cr.execute(
			"select id from learning_programme_unit_standards_learner_rel where selection='t' and learning_programme_id=%s" % (achieved_id.id))
		lp_line_ids = map(lambda x: x[0], self.cr.fetchall())
		dbg(lp_line_ids)
		etqe_lp_us = []
		for us_line in self.pool.get('learning.programme.unit.standards.learner.rel').browse(self.cr, self.uid,
																								  lp_line_ids):
			unit_standard_type.append(us_line.type)
			etqe_lp_us.append(self.pool.get('etqe.learning.programme.unit.standards').search(self.cr, self.uid,
																	[('id_no', '=', us_line.id_no),
																	 ('type', '=', us_line.type)], limit=1)[
					0])
		...
		for u_type in list(set(unit_standard_type)):
			...
			nfq_level = 0
			total_nfq_level = 0
			for lp_line in self.pool.get('etqe.learning.programme.unit.standards').browse(self.cr, self.uid, etqe_lp_us):
	...
	def get_status(self, achieved_id):
		...
		not used in the lqw version
		"""
		dbg("get_status")
		d = {'achieve': False}
		for a_line in achieved_id:
			for s_line in a_line.learning_programme_learner_rel_ids.learning_programme_ids:
...
class lp_statement_of_result_report_view(osv.AbstractModel):
	...
	_template = 'hwseta_etqe.lqw_report_learning_programme_statement_of_result'
	_wrapped_report_class = lqw_learner_lp_statement_of_result
