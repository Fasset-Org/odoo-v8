...
class lqw_learner_qualification_stmt_of_result(report_sxw.rml_parse):

	def __init__(self, cr, uid, name, context=None):
		super(lqw_learner_qualification_stmt_of_result, self).__init__(cr, uid, name, context=context)
	...
	def get_qualification(self, achieved_id):
		...
		"""
		self.cr.execute(
			"select learner_qualification_parent_id from learner_registration_qualification where id=%s" % (achieved_id.id))
	...
	def get_unit_standard(self, achieved_id):
		...
		"""
		dbg("achieved_id" + str(achieved_id))
		...
		# self.cr.execute(
		# 	"select id_data from learner_registration_qualification_line where learner_reg_id=%s" % (
		# 		achieved_id.id))
		self.cr.execute(
			"select id from learner_registration_qualification_line where selection='t' and learner_reg_id=%s" % (
				achieved_id.id))
		qualification_line_ids = map(lambda x: x[0], self.cr.fetchall())
		provider_qualification_line_ids = []
		dbg(qualification_line_ids)
		for qualification_line in self.pool.get('learner.registration.qualification.line').browse(self.cr, self.uid,
																					  qualification_line_ids):
			unit_standard_type.append(qualification_line.type)
			dbg(self.pool.get('provider.qualification.line').search(self.cr, self.uid, [('id_no','=',qualification_line.id_data),('type','=',qualification_line.type)]))
			provider_qualification_line_ids.append(self.pool.get('provider.qualification.line').search(self.cr, self.uid, [('id_no','=',qualification_line.id_data),('type','=',qualification_line.type)],limit=1)[0])
		dbg("provider_qualification_line_ids-" + str(provider_qualification_line_ids))
		total_line = []
		newlist = []
		for u_type in list(set(unit_standard_type)):
			...
			for qualification_line in self.pool.get('provider.qualification.line').browse(self.cr, self.uid, provider_qualification_line_ids):
...
class learner_qualification_sor_report_view(osv.AbstractModel):
	...
	_template = 'hwseta_etqe.lqw_qualification_stmt_of_result_report'
	_wrapped_report_class = lqw_learner_qualification_stmt_of_result
