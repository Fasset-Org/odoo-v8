...
class lqw_learner_lp_achievement_certificate(report_sxw.rml_parse):

    def __init__(self, cr, uid, name, context=None):
        super(lqw_learner_lp_achievement_certificate, self).__init__(
    ...
    def get_lp_nqf_level(self, achieved_id):
        ...
        self.cr.execute(
            "select learning_programme_id from learning_programme_learner_rel where id=%s" % (achieved_id.id))
        lp_ids = map(lambda x: x[0], self.cr.fetchall())
        lp = [str(lp.seta_branch_id.name) for lp in self.pool.get(
            'etqe.learning.programme').browse(self.cr, self.uid, lp_ids)]
        return ','.join(lp)
...
class lp_certificate_report_view(osv.AbstractModel):
    _name = 'report.hwseta_etqe.lqw_report_learning_programme_achievement_certificate'
    _inherit = 'report.abstract_report'
    _template = 'hwseta_etqe.lqw_report_learning_programme_achievement_certificate'
    _wrapped_report_class = lqw_learner_lp_achievement_certificate
