...
'''Python script to update DOB from Identification Number in SDF Master'''

conn_string = "host='localhost' dbname='FASSET' user='odoo' password='odoo'"
conn = psycopg2.connect(conn_string)
cursor = conn.cursor()
output_myfile = open('Invalid DOB in SDF Master.csv', 'wb')
...
cursor.execute(
    "select id, identification_id, person_birth_date from hr_employee where is_sdf = true and citizen_resident_status_code in ('sa', 'dual')")
    ...
    if sdf_id[1] is not None:
        ...
        if int(month) > 12 or int(month) < 1 or int(day) > 31 or int(day) < 1:
            ...
            if birth_date != sdf_id[2]:
                ...
                cursor.execute(
                    "update hr_employee set person_birth_date = %s where is_sdf = True and id = %s", ([birth_date, sdf_id[0]]))
conn.commit()
